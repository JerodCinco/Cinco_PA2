...
public class Decryption {
	 ...
	 */
	 ...
	 //to undo adding 7 and getting the remainder after dividing the new value by 10.
	 
	 if(first_digit <= 6 && first_digit >= 0) {
         first_digit = first_digit + 10;
	 }
     if(second_digit <= 6 && second_digit >= 0) {
         second_digit = second_digit + 10;
     }
     if(third_digit <= 6 && third_digit >= 0) {
         third_digit = third_digit + 10;
     }
     if(fourth_digit <= 6 && fourth_digit >= 0) {
	 ...
	 third_digit = (third_digit - 7);
	 fourth_digit = (fourth_digit - 7);
	 
	 
	 
	 //to undo swapping the first digit with the third, and swap the second digit with the fourth.
	 
	 temp = first_digit;			   //ex. 1420 encrypted is 9781
	 first_digit = third_digit * 1000; // first digit is 1 * 1000 = 1000
	 third_digit = temp * 10;		   // third digit is 2 * 10 = 20
	 
	 temp = second_digit;
	 second_digit = fourth_digit * 100;  
	 fourth_digit = temp;
	 
	 // Then print the decrypted integer.
	 	 
	 System.out.printf("The original integer is %d", first_digit + second_digit + third_digit + fourth_digit);
